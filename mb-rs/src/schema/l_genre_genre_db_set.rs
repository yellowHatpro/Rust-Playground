...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LGenreGenre>> {
        query_as::<_, LGenreGenre>(r#"SELECT * FROM "musicbrainz"."l_genre_genre" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_genre_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, genre_id: i32) -> Result<Vec<LGenreGenre>> {
        query_as::<_, LGenreGenre>(r#"SELECT * FROM "musicbrainz"."l_genre_genre" WHERE entity0 = $1"#)
            .bind(genre_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_genre_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, genre_id: i32) -> Result<Vec<LGenreGenre>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_genre_genre: LGenreGenre) -> Result<LGenreGenre> {
        query_as::<_, LGenreGenre>(r#"UPDATE "l_genre_genre" SET "entity1" = $4, "entity0" = $3, "last_updated" = $6, "entity0_credit" = $8, "entity1_credit" = $9, "edits_pending" = $5, "link_order" = $7, "link" = $2 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_genre_genre.entity0_credit)
            .bind(l_genre_genre.edits_pending)
            .bind(l_genre_genre.entity1_credit)
            .bind(l_genre_genre.last_updated)
