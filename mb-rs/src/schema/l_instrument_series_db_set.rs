...
impl LInstrumentSeriesSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LInstrumentSeries> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LInstrumentSeries> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LInstrumentSeries> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_instrument_id<'e, E: PgExecutor<'e>>(executor: E, instrument_id: i32) -> Result<Vec<LInstrumentSeries>> {
        query_as::<_, LInstrumentSeries>(r#"SELECT * FROM "musicbrainz"."l_instrument_series" WHERE entity0 = $1"#)
            .bind(instrument_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_series_id<'e, E: PgExecutor<'e>>(executor: E, series_id: i32) -> Result<Vec<LInstrumentSeries>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_instrument_series: LInstrumentSeries) -> Result<LInstrumentSeries> {
        query_as::<_, LInstrumentSeries>(r#"UPDATE "l_instrument_series" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_instrument_series.id)
            .bind(l_instrument_series.link)
            .bind(l_instrument_series.entity0)
            .bind(l_instrument_series.entity1)
