...
impl LWorkWorkSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LWorkWork> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LWorkWork> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LWorkWork> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LWorkWork>> {
        query_as::<_, LWorkWork>(r#"SELECT * FROM "musicbrainz"."l_work_work" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_work_id<'e, E: PgExecutor<'e>>(executor: E, work_id: i32) -> Result<Vec<LWorkWork>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_work_work: LWorkWork) -> Result<LWorkWork> {
        query_as::<_, LWorkWork>(r#"UPDATE "l_work_work" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_work_work.id)
            .bind(l_work_work.link)
            .bind(l_work_work.entity0)
            .bind(l_work_work.entity1)
