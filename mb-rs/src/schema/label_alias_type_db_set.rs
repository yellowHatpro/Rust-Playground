...
// SELECT statements by Foreign Key statements
    pub async fn all_by_label_alias_type_id_where_parent_is<'e, E: PgExecutor<'e>>(executor: E, label_alias_type_id: i32) -> Result<Vec<LabelAliasType>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, label_alias_type: LabelAliasType) -> Result<LabelAliasType> {
        query_as::<_, LabelAliasType>(r#"UPDATE "label_alias_type" SET "parent" = $3, "description" = $5, "gid" = $6, "name" = $2, "child_order" = $4 WHERE "id" = 1 RETURNING *;"#)
            .bind(label_alias_type.parent)
            .bind(label_alias_type.name)
            .bind(label_alias_type.id)
            .bind(label_alias_type.child_order)
