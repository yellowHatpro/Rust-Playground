...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LAreaGenre>> {
        query_as::<_, LAreaGenre>(r#"SELECT * FROM "musicbrainz"."l_area_genre" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_area_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, area_id: i32) -> Result<Vec<LAreaGenre>> {
        query_as::<_, LAreaGenre>(r#"SELECT * FROM "musicbrainz"."l_area_genre" WHERE entity0 = $1"#)
            .bind(area_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_genre_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, genre_id: i32) -> Result<Vec<LAreaGenre>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_area_genre: LAreaGenre) -> Result<LAreaGenre> {
        query_as::<_, LAreaGenre>(r#"UPDATE "l_area_genre" SET "entity1" = $4, "last_updated" = $6, "link_order" = $7, "entity0" = $3, "edits_pending" = $5, "entity1_credit" = $9, "entity0_credit" = $8, "link" = $2 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_area_genre.entity1_credit)
            .bind(l_area_genre.last_updated)
            .bind(l_area_genre.link)
            .bind(l_area_genre.entity0_credit)
