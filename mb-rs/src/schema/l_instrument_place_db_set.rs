...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LInstrumentPlace>> {
        query_as::<_, LInstrumentPlace>(r#"SELECT * FROM "musicbrainz"."l_instrument_place" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_instrument_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, instrument_id: i32) -> Result<Vec<LInstrumentPlace>> {
        query_as::<_, LInstrumentPlace>(r#"SELECT * FROM "musicbrainz"."l_instrument_place" WHERE entity0 = $1"#)
            .bind(instrument_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_place_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<LInstrumentPlace>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_instrument_place: LInstrumentPlace) -> Result<LInstrumentPlace> {
        query_as::<_, LInstrumentPlace>(r#"UPDATE "l_instrument_place" SET "link" = $2, "edits_pending" = $5, "entity0_credit" = $8, "entity1_credit" = $9, "entity0" = $3, "entity1" = $4, "last_updated" = $6, "link_order" = $7 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_instrument_place.entity1)
            .bind(l_instrument_place.entity0)
            .bind(l_instrument_place.entity1_credit)
            .bind(l_instrument_place.id)
