...
impl LArtistUrlSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistUrl> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistUrl> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistUrl> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_artist_id<'e, E: PgExecutor<'e>>(executor: E, artist_id: i32) -> Result<Vec<LArtistUrl>> {
        query_as::<_, LArtistUrl>(r#"SELECT * FROM "musicbrainz"."l_artist_url" WHERE entity0 = $1"#)
            .bind(artist_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_url_id<'e, E: PgExecutor<'e>>(executor: E, url_id: i32) -> Result<Vec<LArtistUrl>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_artist_url: LArtistUrl) -> Result<LArtistUrl> {
        query_as::<_, LArtistUrl>(r#"UPDATE "l_artist_url" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_artist_url.id)
            .bind(l_artist_url.link)
            .bind(l_artist_url.entity0)
            .bind(l_artist_url.entity1)
