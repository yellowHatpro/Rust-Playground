...
impl PlaceTypeSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<PlaceType> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<PlaceType>> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<PlaceType>> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<PlaceType> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<PlaceType>> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<PlaceType>> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<PlaceType> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<PlaceType>> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<PlaceType>> {
        query_as::<_, PlaceType>(r#"SELECT * FROM "musicbrainz"."place_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_place_type_id<'e, E: PgExecutor<'e>>(executor: E, place_type_id: i32) -> Result<Vec<PlaceType>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, place_type: PlaceType) -> Result<PlaceType> {
        query_as::<_, PlaceType>(r#"UPDATE "place_type" SET "name" = $2, "parent" = $3, "child_order" = $4, "description" = $5, "gid" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(place_type.id)
            .bind(place_type.name)
            .bind(place_type.parent)
            .bind(place_type.child_order)
