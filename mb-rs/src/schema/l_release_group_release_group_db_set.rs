...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LReleaseGroupReleaseGroup>> {
        query_as::<_, LReleaseGroupReleaseGroup>(r#"SELECT * FROM "musicbrainz"."l_release_group_release_group" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_release_group_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, release_group_id: i32) -> Result<Vec<LReleaseGroupReleaseGroup>> {
        query_as::<_, LReleaseGroupReleaseGroup>(r#"SELECT * FROM "musicbrainz"."l_release_group_release_group" WHERE entity0 = $1"#)
            .bind(release_group_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_release_group_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, release_group_id: i32) -> Result<Vec<LReleaseGroupReleaseGroup>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_release_group_release_group: LReleaseGroupReleaseGroup) -> Result<LReleaseGroupReleaseGroup> {
        query_as::<_, LReleaseGroupReleaseGroup>(r#"UPDATE "l_release_group_release_group" SET "entity0" = $3, "entity1" = $4, "link_order" = $7, "entity1_credit" = $9, "entity0_credit" = $8, "edits_pending" = $5, "link" = $2, "last_updated" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_release_group_release_group.edits_pending)
            .bind(l_release_group_release_group.link_order)
            .bind(l_release_group_release_group.id)
            .bind(l_release_group_release_group.entity0_credit)
