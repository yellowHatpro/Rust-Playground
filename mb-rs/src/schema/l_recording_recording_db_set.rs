...
impl LRecordingRecordingSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LRecordingRecording> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LRecordingRecording> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LRecordingRecording> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LRecordingRecording>> {
        query_as::<_, LRecordingRecording>(r#"SELECT * FROM "musicbrainz"."l_recording_recording" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_recording_id<'e, E: PgExecutor<'e>>(executor: E, recording_id: i32) -> Result<Vec<LRecordingRecording>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_recording_recording: LRecordingRecording) -> Result<LRecordingRecording> {
        query_as::<_, LRecordingRecording>(r#"UPDATE "l_recording_recording" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_recording_recording.id)
            .bind(l_recording_recording.link)
            .bind(l_recording_recording.entity0)
            .bind(l_recording_recording.entity1)
