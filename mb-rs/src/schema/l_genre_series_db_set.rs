...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LGenreSeries>> {
        query_as::<_, LGenreSeries>(r#"SELECT * FROM "musicbrainz"."l_genre_series" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_genre_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, genre_id: i32) -> Result<Vec<LGenreSeries>> {
        query_as::<_, LGenreSeries>(r#"SELECT * FROM "musicbrainz"."l_genre_series" WHERE entity0 = $1"#)
            .bind(genre_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_series_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, series_id: i32) -> Result<Vec<LGenreSeries>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_genre_series: LGenreSeries) -> Result<LGenreSeries> {
        query_as::<_, LGenreSeries>(r#"UPDATE "l_genre_series" SET "edits_pending" = $5, "link" = $2, "entity1" = $4, "link_order" = $7, "last_updated" = $6, "entity0_credit" = $8, "entity1_credit" = $9, "entity0" = $3 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_genre_series.entity0_credit)
            .bind(l_genre_series.link_order)
            .bind(l_genre_series.link)
            .bind(l_genre_series.last_updated)
