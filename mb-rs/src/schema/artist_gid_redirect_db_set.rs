...
// SELECT statements by Foreign Key statements
    pub async fn all_by_artist_id_where_new_id_is<'e, E: PgExecutor<'e>>(executor: E, artist_id: i32) -> Result<Vec<ArtistGidRedirect>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, artist_gid_redirect: ArtistGidRedirect) -> Result<ArtistGidRedirect> {
        query_as::<_, ArtistGidRedirect>(r#"UPDATE "artist_gid_redirect" SET "created" = $3, "new_id" = $2 WHERE "gid" = 1 RETURNING *;"#)
            .bind(artist_gid_redirect.created)
            .bind(artist_gid_redirect.gid)
            .bind(artist_gid_redirect.new_id)
            .fetch_one(executor)
