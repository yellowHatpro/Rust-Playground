...
impl LArtistInstrumentSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistInstrument> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistInstrument> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistInstrument> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_artist_id<'e, E: PgExecutor<'e>>(executor: E, artist_id: i32) -> Result<Vec<LArtistInstrument>> {
        query_as::<_, LArtistInstrument>(r#"SELECT * FROM "musicbrainz"."l_artist_instrument" WHERE entity0 = $1"#)
            .bind(artist_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_instrument_id<'e, E: PgExecutor<'e>>(executor: E, instrument_id: i32) -> Result<Vec<LArtistInstrument>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_artist_instrument: LArtistInstrument) -> Result<LArtistInstrument> {
        query_as::<_, LArtistInstrument>(r#"UPDATE "l_artist_instrument" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_artist_instrument.id)
            .bind(l_artist_instrument.link)
            .bind(l_artist_instrument.entity0)
            .bind(l_artist_instrument.entity1)
