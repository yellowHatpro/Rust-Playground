...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LAreaPlace>> {
        query_as::<_, LAreaPlace>(r#"SELECT * FROM "musicbrainz"."l_area_place" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_area_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, area_id: i32) -> Result<Vec<LAreaPlace>> {
        query_as::<_, LAreaPlace>(r#"SELECT * FROM "musicbrainz"."l_area_place" WHERE entity0 = $1"#)
            .bind(area_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_place_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<LAreaPlace>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_area_place: LAreaPlace) -> Result<LAreaPlace> {
        query_as::<_, LAreaPlace>(r#"UPDATE "l_area_place" SET "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0" = $3, "entity0_credit" = $8, "link" = $2, "entity1" = $4, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_area_place.entity1)
            .bind(l_area_place.link)
            .bind(l_area_place.entity0)
            .bind(l_area_place.id)
