...
impl LAreaEventSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LAreaEvent> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LAreaEvent> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LAreaEvent> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_area_id<'e, E: PgExecutor<'e>>(executor: E, area_id: i32) -> Result<Vec<LAreaEvent>> {
        query_as::<_, LAreaEvent>(r#"SELECT * FROM "musicbrainz"."l_area_event" WHERE entity0 = $1"#)
            .bind(area_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LAreaEvent>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_area_event: LAreaEvent) -> Result<LAreaEvent> {
        query_as::<_, LAreaEvent>(r#"UPDATE "l_area_event" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_area_event.id)
            .bind(l_area_event.link)
            .bind(l_area_event.entity0)
            .bind(l_area_event.entity1)
