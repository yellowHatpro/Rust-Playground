...
impl LSeriesUrlSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LSeriesUrl> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LSeriesUrl> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LSeriesUrl> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_series_id<'e, E: PgExecutor<'e>>(executor: E, series_id: i32) -> Result<Vec<LSeriesUrl>> {
        query_as::<_, LSeriesUrl>(r#"SELECT * FROM "musicbrainz"."l_series_url" WHERE entity0 = $1"#)
            .bind(series_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_url_id<'e, E: PgExecutor<'e>>(executor: E, url_id: i32) -> Result<Vec<LSeriesUrl>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_series_url: LSeriesUrl) -> Result<LSeriesUrl> {
        query_as::<_, LSeriesUrl>(r#"UPDATE "l_series_url" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_series_url.id)
            .bind(l_series_url.link)
            .bind(l_series_url.entity0)
            .bind(l_series_url.entity1)
