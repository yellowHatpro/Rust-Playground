...
impl LEventGenreSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LEventGenre> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LEventGenre> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LEventGenre> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LEventGenre>> {
        query_as::<_, LEventGenre>(r#"SELECT * FROM "musicbrainz"."l_event_genre" WHERE entity0 = $1"#)
            .bind(event_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_genre_id<'e, E: PgExecutor<'e>>(executor: E, genre_id: i32) -> Result<Vec<LEventGenre>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_event_genre: LEventGenre) -> Result<LEventGenre> {
        query_as::<_, LEventGenre>(r#"UPDATE "l_event_genre" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_event_genre.id)
            .bind(l_event_genre.link)
            .bind(l_event_genre.entity0)
            .bind(l_event_genre.entity1)
