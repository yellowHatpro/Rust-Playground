...
impl GenreAliasSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<GenreAlias> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<GenreAlias> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<GenreAlias> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_genre_id<'e, E: PgExecutor<'e>>(executor: E, genre_id: i32) -> Result<Vec<GenreAlias>> {
        query_as::<_, GenreAlias>(r#"SELECT * FROM "musicbrainz"."genre_alias" WHERE genre = $1"#)
            .bind(genre_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_genre_alias_type_id<'e, E: PgExecutor<'e>>(executor: E, genre_alias_type_id: i32) -> Result<Vec<GenreAlias>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, genre_alias: GenreAlias) -> Result<GenreAlias> {
        query_as::<_, GenreAlias>(r#"UPDATE "genre_alias" SET "genre" = $2, "name" = $3, "locale" = $4, "edits_pending" = $5, "last_updated" = $6, "type" = $7, "sort_name" = $8, "begin_date_year" = $9, "begin_date_month" = $10, "begin_date_day" = $11, "end_date_year" = $12, "end_date_month" = $13, "end_date_day" = $14, "primary_for_locale" = $15, "ended" = $16 WHERE "id" = 1 RETURNING *;"#)
            .bind(genre_alias.id)
            .bind(genre_alias.genre)
            .bind(genre_alias.name)
            .bind(genre_alias.locale)
