...
impl EditorCollectionTypeSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<EditorCollectionType> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<EditorCollectionType>> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<EditorCollectionType>> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<EditorCollectionType> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<EditorCollectionType>> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<EditorCollectionType>> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<EditorCollectionType> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<EditorCollectionType>> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<EditorCollectionType>> {
        query_as::<_, EditorCollectionType>(r#"SELECT * FROM "musicbrainz"."editor_collection_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_editor_collection_type_id<'e, E: PgExecutor<'e>>(executor: E, editor_collection_type_id: i32) -> Result<Vec<EditorCollectionType>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, editor_collection_type: EditorCollectionType) -> Result<EditorCollectionType> {
        query_as::<_, EditorCollectionType>(r#"UPDATE "editor_collection_type" SET "name" = $2, "entity_type" = $3, "parent" = $4, "child_order" = $5, "description" = $6, "gid" = $7 WHERE "id" = 1 RETURNING *;"#)
            .bind(editor_collection_type.id)
            .bind(editor_collection_type.name)
            .bind(editor_collection_type.entity_type)
            .bind(editor_collection_type.parent)
