...
impl LArtistReleaseSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistRelease> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistRelease> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LArtistRelease> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_artist_id<'e, E: PgExecutor<'e>>(executor: E, artist_id: i32) -> Result<Vec<LArtistRelease>> {
        query_as::<_, LArtistRelease>(r#"SELECT * FROM "musicbrainz"."l_artist_release" WHERE entity0 = $1"#)
            .bind(artist_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_release_id<'e, E: PgExecutor<'e>>(executor: E, release_id: i32) -> Result<Vec<LArtistRelease>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_artist_release: LArtistRelease) -> Result<LArtistRelease> {
        query_as::<_, LArtistRelease>(r#"UPDATE "l_artist_release" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_artist_release.id)
            .bind(l_artist_release.link)
            .bind(l_artist_release.entity0)
            .bind(l_artist_release.entity1)
