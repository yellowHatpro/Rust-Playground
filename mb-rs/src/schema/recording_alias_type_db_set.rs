...
// SELECT statements by Foreign Key statements
    pub async fn all_by_recording_alias_type_id_where_parent_is<'e, E: PgExecutor<'e>>(executor: E, recording_alias_type_id: i32) -> Result<Vec<RecordingAliasType>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, recording_alias_type: RecordingAliasType) -> Result<RecordingAliasType> {
        query_as::<_, RecordingAliasType>(r#"UPDATE "recording_alias_type" SET "child_order" = $4, "name" = $2, "parent" = $3, "description" = $5, "gid" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(recording_alias_type.description)
            .bind(recording_alias_type.id)
            .bind(recording_alias_type.parent)
            .bind(recording_alias_type.gid)
