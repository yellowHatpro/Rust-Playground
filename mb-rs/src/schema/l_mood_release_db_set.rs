...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LMoodRelease>> {
        query_as::<_, LMoodRelease>(r#"SELECT * FROM "musicbrainz"."l_mood_release" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_mood_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, mood_id: i32) -> Result<Vec<LMoodRelease>> {
        query_as::<_, LMoodRelease>(r#"SELECT * FROM "musicbrainz"."l_mood_release" WHERE entity0 = $1"#)
            .bind(mood_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_release_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, release_id: i32) -> Result<Vec<LMoodRelease>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_mood_release: LMoodRelease) -> Result<LMoodRelease> {
        query_as::<_, LMoodRelease>(r#"UPDATE "l_mood_release" SET "link" = $2, "link_order" = $7, "entity0_credit" = $8, "entity1" = $4, "last_updated" = $6, "edits_pending" = $5, "entity1_credit" = $9, "entity0" = $3 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_mood_release.entity0_credit)
            .bind(l_mood_release.entity1)
            .bind(l_mood_release.last_updated)
            .bind(l_mood_release.link_order)
