...
impl SeriesOrderingTypeSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<SeriesOrderingType> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<SeriesOrderingType>> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<SeriesOrderingType>> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<SeriesOrderingType> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<SeriesOrderingType>> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<SeriesOrderingType>> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<SeriesOrderingType> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<SeriesOrderingType>> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<SeriesOrderingType>> {
        query_as::<_, SeriesOrderingType>(r#"SELECT * FROM "musicbrainz"."series_ordering_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_series_ordering_type_id<'e, E: PgExecutor<'e>>(executor: E, series_ordering_type_id: i32) -> Result<Vec<SeriesOrderingType>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, series_ordering_type: SeriesOrderingType) -> Result<SeriesOrderingType> {
        query_as::<_, SeriesOrderingType>(r#"UPDATE "series_ordering_type" SET "name" = $2, "parent" = $3, "child_order" = $4, "description" = $5, "gid" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(series_ordering_type.id)
            .bind(series_ordering_type.name)
            .bind(series_ordering_type.parent)
            .bind(series_ordering_type.child_order)
