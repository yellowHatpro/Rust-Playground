...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LArtistLabel>> {
        query_as::<_, LArtistLabel>(r#"SELECT * FROM "musicbrainz"."l_artist_label" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_artist_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, artist_id: i32) -> Result<Vec<LArtistLabel>> {
        query_as::<_, LArtistLabel>(r#"SELECT * FROM "musicbrainz"."l_artist_label" WHERE entity0 = $1"#)
            .bind(artist_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_label_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, label_id: i32) -> Result<Vec<LArtistLabel>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_artist_label: LArtistLabel) -> Result<LArtistLabel> {
        query_as::<_, LArtistLabel>(r#"UPDATE "l_artist_label" SET "edits_pending" = $5, "entity1" = $4, "last_updated" = $6, "entity1_credit" = $9, "entity0" = $3, "link" = $2, "entity0_credit" = $8, "link_order" = $7 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_artist_label.entity0_credit)
            .bind(l_artist_label.entity1_credit)
            .bind(l_artist_label.entity1)
            .bind(l_artist_label.entity0)
