...
impl ReleaseAliasTypeSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<ReleaseAliasType> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<ReleaseAliasType>> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<ReleaseAliasType>> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<ReleaseAliasType> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<ReleaseAliasType>> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<ReleaseAliasType>> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<ReleaseAliasType> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<ReleaseAliasType>> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<ReleaseAliasType>> {
        query_as::<_, ReleaseAliasType>(r#"SELECT * FROM "musicbrainz"."release_alias_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_release_alias_type_id<'e, E: PgExecutor<'e>>(executor: E, release_alias_type_id: i32) -> Result<Vec<ReleaseAliasType>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, release_alias_type: ReleaseAliasType) -> Result<ReleaseAliasType> {
        query_as::<_, ReleaseAliasType>(r#"UPDATE "release_alias_type" SET "name" = $2, "parent" = $3, "child_order" = $4, "description" = $5, "gid" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(release_alias_type.id)
            .bind(release_alias_type.name)
            .bind(release_alias_type.parent)
            .bind(release_alias_type.child_order)
