...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LPlacePlace>> {
        query_as::<_, LPlacePlace>(r#"SELECT * FROM "musicbrainz"."l_place_place" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_place_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<LPlacePlace>> {
        query_as::<_, LPlacePlace>(r#"SELECT * FROM "musicbrainz"."l_place_place" WHERE entity0 = $1"#)
            .bind(place_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_place_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<LPlacePlace>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_place_place: LPlacePlace) -> Result<LPlacePlace> {
        query_as::<_, LPlacePlace>(r#"UPDATE "l_place_place" SET "link_order" = $7, "entity1_credit" = $9, "edits_pending" = $5, "link" = $2, "entity0" = $3, "entity0_credit" = $8, "last_updated" = $6, "entity1" = $4 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_place_place.link)
            .bind(l_place_place.entity0)
            .bind(l_place_place.id)
            .bind(l_place_place.last_updated)
