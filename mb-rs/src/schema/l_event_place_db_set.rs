...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LEventPlace>> {
        query_as::<_, LEventPlace>(r#"SELECT * FROM "musicbrainz"."l_event_place" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LEventPlace>> {
        query_as::<_, LEventPlace>(r#"SELECT * FROM "musicbrainz"."l_event_place" WHERE entity0 = $1"#)
            .bind(event_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_place_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<LEventPlace>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_event_place: LEventPlace) -> Result<LEventPlace> {
        query_as::<_, LEventPlace>(r#"UPDATE "l_event_place" SET "entity0" = $3, "entity0_credit" = $8, "entity1" = $4, "last_updated" = $6, "link" = $2, "link_order" = $7, "entity1_credit" = $9, "edits_pending" = $5 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_event_place.link_order)
            .bind(l_event_place.entity1_credit)
            .bind(l_event_place.id)
            .bind(l_event_place.entity1)
