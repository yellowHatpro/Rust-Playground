...
impl WorkTagRawSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<WorkTagRaw> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<WorkTagRaw> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<WorkTagRaw> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_work_id<'e, E: PgExecutor<'e>>(executor: E, work_id: i32) -> Result<Vec<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE work = $1"#)
            .bind(work_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_editor_id<'e, E: PgExecutor<'e>>(executor: E, editor_id: i32) -> Result<Vec<WorkTagRaw>> {
        query_as::<_, WorkTagRaw>(r#"SELECT * FROM "musicbrainz"."work_tag_raw" WHERE editor = $1"#)
            .bind(editor_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_tag_id<'e, E: PgExecutor<'e>>(executor: E, tag_id: i32) -> Result<Vec<WorkTagRaw>> {
