...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LLabelRelease>> {
        query_as::<_, LLabelRelease>(r#"SELECT * FROM "musicbrainz"."l_label_release" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_label_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, label_id: i32) -> Result<Vec<LLabelRelease>> {
        query_as::<_, LLabelRelease>(r#"SELECT * FROM "musicbrainz"."l_label_release" WHERE entity0 = $1"#)
            .bind(label_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_release_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, release_id: i32) -> Result<Vec<LLabelRelease>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_label_release: LLabelRelease) -> Result<LLabelRelease> {
        query_as::<_, LLabelRelease>(r#"UPDATE "l_label_release" SET "entity0" = $3, "link" = $2, "entity0_credit" = $8, "entity1" = $4, "edits_pending" = $5, "entity1_credit" = $9, "last_updated" = $6, "link_order" = $7 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_label_release.entity1_credit)
            .bind(l_label_release.last_updated)
            .bind(l_label_release.entity1)
            .bind(l_label_release.link_order)
