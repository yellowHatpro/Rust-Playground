...
impl LAreaWorkSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LAreaWork> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LAreaWork> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LAreaWork> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_area_id<'e, E: PgExecutor<'e>>(executor: E, area_id: i32) -> Result<Vec<LAreaWork>> {
        query_as::<_, LAreaWork>(r#"SELECT * FROM "musicbrainz"."l_area_work" WHERE entity0 = $1"#)
            .bind(area_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_work_id<'e, E: PgExecutor<'e>>(executor: E, work_id: i32) -> Result<Vec<LAreaWork>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_area_work: LAreaWork) -> Result<LAreaWork> {
        query_as::<_, LAreaWork>(r#"UPDATE "l_area_work" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_area_work.id)
            .bind(l_area_work.link)
            .bind(l_area_work.entity0)
            .bind(l_area_work.entity1)
