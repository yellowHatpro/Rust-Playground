...
// SELECT statements by Foreign Key statements
    pub async fn all_by_place_id_where_place_is<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<PlaceTagRaw>> {
        query_as::<_, PlaceTagRaw>(r#"SELECT * FROM "musicbrainz"."place_tag_raw" WHERE place = $1"#)
            .bind(place_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_editor_id_where_editor_is<'e, E: PgExecutor<'e>>(executor: E, editor_id: i32) -> Result<Vec<PlaceTagRaw>> {
        query_as::<_, PlaceTagRaw>(r#"SELECT * FROM "musicbrainz"."place_tag_raw" WHERE editor = $1"#)
            .bind(editor_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_tag_id_where_tag_is<'e, E: PgExecutor<'e>>(executor: E, tag_id: i32) -> Result<Vec<PlaceTagRaw>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, place_tag_raw: PlaceTagRaw) -> Result<PlaceTagRaw> {
        query_as::<_, PlaceTagRaw>(r#"UPDATE "place_tag_raw" SET "is_upvote" = $4 WHERE "editor" = 2 AND "tag" = 3 AND "place" = 1 RETURNING *;"#)
            .bind(place_tag_raw.tag)
            .bind(place_tag_raw.editor)
            .bind(place_tag_raw.is_upvote)
            .bind(place_tag_raw.place)
...
// DELETE statements
    pub async fn delete<'e, E: PgExecutor<'e>>(&self, executor: E) -> Result<()> {
        query(r#"DELETE FROM "musicbrainz"."place_tag_raw" WHERE "tag" = 3 AND "place" = 1 AND "editor" = 2"#)
            .execute(executor)
            .await
            .map(|_| ())
    }
