...
impl AreaAliasSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<AreaAlias> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<AreaAlias> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<AreaAlias> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_area_id<'e, E: PgExecutor<'e>>(executor: E, area_id: i32) -> Result<Vec<AreaAlias>> {
        query_as::<_, AreaAlias>(r#"SELECT * FROM "musicbrainz"."area_alias" WHERE area = $1"#)
            .bind(area_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_area_alias_type_id<'e, E: PgExecutor<'e>>(executor: E, area_alias_type_id: i32) -> Result<Vec<AreaAlias>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, area_alias: AreaAlias) -> Result<AreaAlias> {
        query_as::<_, AreaAlias>(r#"UPDATE "area_alias" SET "area" = $2, "name" = $3, "locale" = $4, "edits_pending" = $5, "last_updated" = $6, "type" = $7, "sort_name" = $8, "begin_date_year" = $9, "begin_date_month" = $10, "begin_date_day" = $11, "end_date_year" = $12, "end_date_month" = $13, "end_date_day" = $14, "primary_for_locale" = $15, "ended" = $16 WHERE "id" = 1 RETURNING *;"#)
            .bind(area_alias.id)
            .bind(area_alias.area)
            .bind(area_alias.name)
            .bind(area_alias.locale)
