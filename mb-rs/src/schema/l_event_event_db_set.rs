...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LEventEvent>> {
        query_as::<_, LEventEvent>(r#"SELECT * FROM "musicbrainz"."l_event_event" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LEventEvent>> {
        query_as::<_, LEventEvent>(r#"SELECT * FROM "musicbrainz"."l_event_event" WHERE entity0 = $1"#)
            .bind(event_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LEventEvent>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_event_event: LEventEvent) -> Result<LEventEvent> {
        query_as::<_, LEventEvent>(r#"UPDATE "l_event_event" SET "last_updated" = $6, "edits_pending" = $5, "link" = $2, "entity0" = $3, "entity1" = $4, "entity1_credit" = $9, "link_order" = $7, "entity0_credit" = $8 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_event_event.entity0)
            .bind(l_event_event.id)
            .bind(l_event_event.link_order)
            .bind(l_event_event.last_updated)
