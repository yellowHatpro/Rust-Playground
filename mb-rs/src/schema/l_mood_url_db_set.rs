...
impl LMoodUrlSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LMoodUrl> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LMoodUrl> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LMoodUrl> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_mood_id<'e, E: PgExecutor<'e>>(executor: E, mood_id: i32) -> Result<Vec<LMoodUrl>> {
        query_as::<_, LMoodUrl>(r#"SELECT * FROM "musicbrainz"."l_mood_url" WHERE entity0 = $1"#)
            .bind(mood_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_url_id<'e, E: PgExecutor<'e>>(executor: E, url_id: i32) -> Result<Vec<LMoodUrl>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_mood_url: LMoodUrl) -> Result<LMoodUrl> {
        query_as::<_, LMoodUrl>(r#"UPDATE "l_mood_url" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_mood_url.id)
            .bind(l_mood_url.link)
            .bind(l_mood_url.entity0)
            .bind(l_mood_url.entity1)
