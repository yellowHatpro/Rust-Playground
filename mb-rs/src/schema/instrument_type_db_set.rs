...
impl InstrumentTypeSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<InstrumentType> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<InstrumentType>> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<InstrumentType>> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<InstrumentType> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<InstrumentType>> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<InstrumentType>> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<InstrumentType> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<InstrumentType>> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<InstrumentType>> {
        query_as::<_, InstrumentType>(r#"SELECT * FROM "musicbrainz"."instrument_type" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_instrument_type_id<'e, E: PgExecutor<'e>>(executor: E, instrument_type_id: i32) -> Result<Vec<InstrumentType>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, instrument_type: InstrumentType) -> Result<InstrumentType> {
        query_as::<_, InstrumentType>(r#"UPDATE "instrument_type" SET "name" = $2, "parent" = $3, "child_order" = $4, "description" = $5, "gid" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(instrument_type.id)
            .bind(instrument_type.name)
            .bind(instrument_type.parent)
            .bind(instrument_type.child_order)
