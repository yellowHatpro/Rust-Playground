...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LMoodRecording>> {
        query_as::<_, LMoodRecording>(r#"SELECT * FROM "musicbrainz"."l_mood_recording" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_mood_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, mood_id: i32) -> Result<Vec<LMoodRecording>> {
        query_as::<_, LMoodRecording>(r#"SELECT * FROM "musicbrainz"."l_mood_recording" WHERE entity0 = $1"#)
            .bind(mood_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_recording_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, recording_id: i32) -> Result<Vec<LMoodRecording>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_mood_recording: LMoodRecording) -> Result<LMoodRecording> {
        query_as::<_, LMoodRecording>(r#"UPDATE "l_mood_recording" SET "entity1_credit" = $9, "entity1" = $4, "entity0_credit" = $8, "link" = $2, "edits_pending" = $5, "entity0" = $3, "link_order" = $7, "last_updated" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_mood_recording.entity1_credit)
            .bind(l_mood_recording.entity0_credit)
            .bind(l_mood_recording.link_order)
            .bind(l_mood_recording.link)
