...
impl LReleaseWorkSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LReleaseWork> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LReleaseWork> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LReleaseWork> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_release_id<'e, E: PgExecutor<'e>>(executor: E, release_id: i32) -> Result<Vec<LReleaseWork>> {
        query_as::<_, LReleaseWork>(r#"SELECT * FROM "musicbrainz"."l_release_work" WHERE entity0 = $1"#)
            .bind(release_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_work_id<'e, E: PgExecutor<'e>>(executor: E, work_id: i32) -> Result<Vec<LReleaseWork>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_release_work: LReleaseWork) -> Result<LReleaseWork> {
        query_as::<_, LReleaseWork>(r#"UPDATE "l_release_work" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_release_work.id)
            .bind(l_release_work.link)
            .bind(l_release_work.entity0)
            .bind(l_release_work.entity1)
