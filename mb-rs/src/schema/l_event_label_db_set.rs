...
impl LEventLabelSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LEventLabel> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LEventLabel> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<LEventLabel> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_link_id<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LEventLabel>> {
        query_as::<_, LEventLabel>(r#"SELECT * FROM "musicbrainz"."l_event_label" WHERE entity0 = $1"#)
            .bind(event_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_label_id<'e, E: PgExecutor<'e>>(executor: E, label_id: i32) -> Result<Vec<LEventLabel>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_event_label: LEventLabel) -> Result<LEventLabel> {
        query_as::<_, LEventLabel>(r#"UPDATE "l_event_label" SET "link" = $2, "entity0" = $3, "entity1" = $4, "edits_pending" = $5, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_event_label.id)
            .bind(l_event_label.link)
            .bind(l_event_label.entity0)
            .bind(l_event_label.entity1)
