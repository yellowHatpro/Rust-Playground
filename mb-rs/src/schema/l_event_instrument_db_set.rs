...
// SELECT statements by Foreign Key statements
    pub async fn all_by_link_id_where_link_is<'e, E: PgExecutor<'e>>(executor: E, link_id: i32) -> Result<Vec<LEventInstrument>> {
        query_as::<_, LEventInstrument>(r#"SELECT * FROM "musicbrainz"."l_event_instrument" WHERE link = $1"#)
            .bind(link_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_event_id_where_entity0_is<'e, E: PgExecutor<'e>>(executor: E, event_id: i32) -> Result<Vec<LEventInstrument>> {
        query_as::<_, LEventInstrument>(r#"SELECT * FROM "musicbrainz"."l_event_instrument" WHERE entity0 = $1"#)
            .bind(event_id)
            .fetch_all(executor)
            .await
    }
    pub async fn all_by_instrument_id_where_entity1_is<'e, E: PgExecutor<'e>>(executor: E, instrument_id: i32) -> Result<Vec<LEventInstrument>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, l_event_instrument: LEventInstrument) -> Result<LEventInstrument> {
        query_as::<_, LEventInstrument>(r#"UPDATE "l_event_instrument" SET "entity0" = $3, "entity1" = $4, "last_updated" = $6, "link_order" = $7, "entity0_credit" = $8, "entity1_credit" = $9, "link" = $2, "edits_pending" = $5 WHERE "id" = 1 RETURNING *;"#)
            .bind(l_event_instrument.edits_pending)
            .bind(l_event_instrument.id)
            .bind(l_event_instrument.link)
            .bind(l_event_instrument.entity1_credit)
