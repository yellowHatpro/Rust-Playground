...
impl PlaceGidRedirectSet {
    ...
    pub async fn by_release<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<PlaceGidRedirect> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_release_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<PlaceGidRedirect>> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_release_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<PlaceGidRedirect>> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_position<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<PlaceGidRedirect> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_position_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<PlaceGidRedirect>> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_position_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<PlaceGidRedirect>> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_optional(executor)
            .await
    }

    pub async fn by_medium<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<PlaceGidRedirect> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_one(executor)
            .await
    }

    pub async fn many_by_medium_list<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Vec<PlaceGidRedirect>> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_all(executor)
            .await
    }

    pub async fn by_medium_optional<'e, E: PgExecutor<'e>>(&self, executor: E, ) -> Result<Option<PlaceGidRedirect>> {
        query_as::<_, PlaceGidRedirect>(r#"SELECT * FROM "musicbrainz"."place_gid_redirect" WHERE "#)
            .fetch_optional(executor)
            .await
    }
    ...
    pub async fn all_by_place_id<'e, E: PgExecutor<'e>>(executor: E, place_id: i32) -> Result<Vec<PlaceGidRedirect>> {
    ...
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, place_gid_redirect: PlaceGidRedirect) -> Result<PlaceGidRedirect> {
        query_as::<_, PlaceGidRedirect>(r#"UPDATE "place_gid_redirect" SET "new_id" = $2, "created" = $3 WHERE "gid" = 1 RETURNING *;"#)
            .bind(place_gid_redirect.gid)
            .bind(place_gid_redirect.new_id)
            .bind(place_gid_redirect.created)
            .fetch_one(executor)
