...
// SELECT statements by Foreign Key statements
    pub async fn all_by_genre_alias_type_id_where_parent_is<'e, E: PgExecutor<'e>>(executor: E, genre_alias_type_id: i32) -> Result<Vec<GenreAliasType>> {
...
// UPDATE statements
    pub async fn update<'e, E: PgExecutor<'e>>(&self, executor: E, genre_alias_type: GenreAliasType) -> Result<GenreAliasType> {
        query_as::<_, GenreAliasType>(r#"UPDATE "genre_alias_type" SET "parent" = $3, "name" = $2, "description" = $5, "child_order" = $4, "gid" = $6 WHERE "id" = 1 RETURNING *;"#)
            .bind(genre_alias_type.parent)
            .bind(genre_alias_type.description)
            .bind(genre_alias_type.name)
            .bind(genre_alias_type.child_order)
